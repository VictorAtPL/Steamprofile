	...
	this.init = function() {		
		...
		jQuery.ajax({
			...
			complete: function(request, status) {
				configData = jQuery(request.responseXML);
				loadConfig();
			}
		});
	};
	...
	this.load = function(profileID) {
		...
		jQuery.ajax({
			...
			complete: function(request, status) {
				...
				profile.empty().append(createProfile(jQuery(request.responseXML)));
			}
		});
		...
		if(profileCache[profileID] == null) {
			...
			jQuery.ajax({
				...
				complete: function(request, status) {
					...
					profileCache[profileID] = createProfile(jQuery(request.responseXML));
